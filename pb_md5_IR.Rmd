...
#load pheno_2020 from Andrew J.C. Russell et.al. as metadata [RStudio](https://www.rstudio.com)
...
# call differentially retained introns, p-value adjusted by FDR method
intron_diff_fdr <- FindMarkers(intron_md5, group.by = "intron_cluster",
                              ident.1 = "md5_male", ident.2 = "wt_male",
                              logfc.threshold=0, min.pct=0, min.diff.pct=0)

intron_diff_fdr$fdr = p.adjust(intron_diff_fdr$p_val, method='fdr')
rownames(intron_diff_fdr) <- gsub("-", "_", rownames(intron_diff_fdr), fix = T)
intron_diff_fdr$gene_id <- str_sub(rownames(intron_diff_fdr), 1, 14)
write.csv(intron_diff_fdr, "intron_diff_ff_fdr.csv")

intron_diff_logFC1_fdr_0.05 <- filter(intron_diff_fdr,
                                      avg_log2FC >= 1,
                                      fdr <= 0.05)

write.csv(intron_diff_logFC1_fdr_0.05, "intron_diff_fdr_logFC1_fdr_0.05.csv")
...
pb_ortho_padj <- pb_ortho_padj[,c(1,7)]
...
vol_data <- mutate(vol_data, color = case_when(vol_data$lfc > 1 & vol_data$padj > 1.3  ~ "Increased",
                                       vol_data$lfc < -1 & vol_data$padj > 1.3  ~ "Decreased",
                                       vol_data$padj < 1.3  ~ "nonsignificant",
                                       vol_data$lfc <= 1 & vol_data$padj >= -1 ~ "nonsignificant"
                                      ))
...
vol +   
     ggtitle(label = "Volcano Plot for Intron Rentetion Events", subtitle = "Colored by fold-change direction") +
             ...
             geom_text_repel(aes(label = intron), 
                       data = vol_data[vol_data$intron  %in% pb_co_list,],size= 4, 
                       vjust= -1, hjust = 1, show.legend = FALSE, color="black") +
             geom_point(data = vol_data[vol_data$intron  %in% pb_co_list,],color="#33bfff") +
             theme_bw() + 
